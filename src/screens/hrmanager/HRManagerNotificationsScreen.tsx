...
import { hrService, type HRNotificationItem } from '../../services/hrService';

const HRManagerNotificationsScreen: React.FC = () => {
  const navigation = useNavigation<any>();
  const { t } = useTranslation();
  const [list, setList] = useState<HRNotificationItem[]>([]);
  ...
  const handleMarkSelectedAsRead = useCallback(async () => {
    ...
    try {
      await Promise.all(selectedIds.map((id) => hrService.markNotificationAsRead(id)));
  ...
  const handleMarkAllAsRead = useCallback(async () => {
    ...
    setMarkingAll(true);
    try {
      const res = await hrService.markAllNotificationsAsRead();
  ...
  const handleDeleteSelected = useCallback(async () => {
    ...
    try {
      await Promise.all(selectedIds.map((id) => hrService.deleteNotification(id)));
  ...
  const handleClearAll = useCallback(async () => {
    ...
    setClearingAll(true);
    try {
      const res = await hrService.clearAllNotifications();
  ...
  const renderNotification = ({ item }: { item: HRNotificationItem }) => {
...
export default HRManagerNotificationsScreen;
