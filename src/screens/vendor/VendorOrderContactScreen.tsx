...
export default function VendorOrderContactScreen() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      Alert.alert('Error', error instanceof Error ? error.message : 'Failed to load contact.');
      setContact(null);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      <VendorPageHeader title="Contact Customer" showBack />
      ...
      {loading ? (
        ...
        <View style={styles.content}>
          ...
          <View style={styles.actions}>
            {callUrl ? (
              <TouchableOpacity style={[styles.actionBtn, styles.callBtn]} onPress={() => openUrl(callUrl, 'phone')}>
            ...
            {whatsappUrl ? (
              <TouchableOpacity
                ...
                onPress={() => openUrl(whatsappUrl, 'WhatsApp')}
              >
            ...
            {mailtoUrl ? (
              <TouchableOpacity style={[styles.actionBtn, styles.emailBtn]} onPress={() => openUrl(mailtoUrl, 'email')}>
          ...
          {!callUrl && !whatsappUrl && !mailtoUrl ? (
            <Text style={styles.noContact}>No contact methods available for this order.</Text>
          ) : null}
        </View>
      ) : (
        <View style={styles.centered}>
          <Text style={styles.noContact}>Contact details unavailable.</Text>
        </View>
      )}
    </SafeAreaView>
  );
...
const styles = StyleSheet.create({
  ...
  noContact: { fontSize: FONT_SIZES.sm, color: COLORS.textSecondary, textAlign: 'center', marginTop: SPACING.lg },
});
